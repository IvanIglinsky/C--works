    ...
    public partial class Server : Form
    {
        ...
        private void AddToGrid(long id, string name)
        {
            if (!exit)
            {
                clientsDataGridView.Invoke((MethodInvoker)delegate
                {
                    ...
                    totalLabel.Text = string.Format("Всі клієнти: {0}", clientsDataGridView.Rows.Count);
                });
            }
        }
        ...
        private void Connection(MyClient obj)
        {
            if (Authorize(obj))
            {
                ...
                string msg = string.Format("{0} приєднано", obj.username);
                Log(SystemMsg(msg));
                ...
                msg = string.Format("{0} відєднено", tmp.username);
                Log(SystemMsg(msg));
                Send(msg, tmp.id);
            }
        }
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            else if (listener == null || !listener.IsAlive)
            {
                ...
                IPAddress ip = null;             
                {
                    ...
                    {
                        error = true;
                        Log(SystemMsg("Адреса не правильна"));
                    }
                }
                int port = -1;
                if (number.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Порт не правильний"));
                }
                ...
                {
                    error = true;
                    Log(SystemMsg("Порт виходить за межи"));
                }
                if (username.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Неправильний нік"));
                }
                if (!error)
                {
        ...
        private void SendTextBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (sendTextBox.Text.Length > 0)
                {
                    ...
                    sendTextBox.Clear();
                    Log(string.Format("{0} (Ви): {1}", usernameTextBox.Text.Trim(), msg));
